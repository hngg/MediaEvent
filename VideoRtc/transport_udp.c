...
ssize_t channel_recv_loop(struct trans_channel*recv_channel, void *user_udp)
{
    ...
    on_recv_data recv_cb = recv_channel->recv_cb;
    while(recv_channel->sockfd != PJ_INVALID_SOCKET && !recv_channel->recv_tid.thread_quit)  /* modify by j33783 20190509 */
    {
        ...
        if(recv_channel->is_rtcp)
        {
            ...
            if(tv_end.tv_sec-tv_start.tv_sec>=1)
            {
                ...
                if(user_udp)
                {
                    ...
                    log_debug("rtcp send begin size:%d rtcp:%d addr:%s port:%d", size, channel.is_rtcp, 
                                inet_ntoa(channel.remote_addr.sin_addr), htons(channel.remote_addr.sin_port));
                }
            }
        }//rtcp sr heartbeat
        ...
        if(status>0)
        {
           if(FD_ISSET(recv_channel->sockfd, &recv_set))
           {
               ...
               if (status>0 && recv_cb) 
               {
                   (*recv_cb)(udp->user_stream, recv_channel->recv_buf, status);
                   if(recv_channel->is_rtcp)
                        log_debug("rtcp recvfrom H264__ size:%d ", status);
               }
               else//EAGAIN
               {
                   //to do
...
static void worker_rtp_recvfrom(void *arg)
{
    ...
    log_debug("running rtp recv_from thread exit status:%d threadid:%ld", status, recv_channel->recv_tid.threadId);
...
static void worker_rtcp_recvfrom(void *arg)
{
    ...
    struct transport_udp *udp       = (struct transport_udp *) arg;
    struct trans_channel *channel   = &udp->rtcp_chanel;

    log_debug("running rtcp recv_from thread %ld begin.", pthread_self());  /* modify by j33783 20190509 */

    pj_uint32_t  value = 0;
    if (ioctl(channel->sockfd, FIONBIO, &value))
    {
        //status  = pj_get_netos_error();
        return ;
    }

    status = channel_recv_loop(channel, udp);
    log_debug("running rtcp recv_from thread exit status:%d threadid:%ld", status, channel->recv_tid.threadId);
...
//for rtp
void resend_save_rtp( pjmedia_vid_buf *resend_buf, pj_uint16_t extSeq, char*sendBuf, pj_uint16_t pkt_len)
{
	if(!resend_buf || !sendBuf)
		return ;
    
    if(resend_buf->buf && resend_buf->pkt_len) 
    {
        pj_uint16_t index;
        index = extSeq % resend_buf->buf_size;
        pj_memcpy(resend_buf->buf + index * PJMEDIA_MAX_MTU, sendBuf, pkt_len); //pkt_len is smaller than PJMEDIA_MAX_MTU
        resend_buf->pkt_len[index] = pkt_len;
    }
}
...
pj_status_t resend_losted_package( struct transport_udp* udp, unsigned begin_seq, unsigned count)
{
    ...
    for(; i<count; i++)
    {
        ...
        if(status < 0)
        {
            ...
            log_error("send_priority resend failed sockid:%d seq:%d errno:%d", rt_channel->sockfd, seq-1, errno);
        }
        else
                                     ...
                                     pj_bool_t is_rtcp, on_recv_data recv_cb, func_worker_recvfrom work_recv)
{
    ...
    struct sockaddr_in *bind_addr = &chan->local_addr;
    memset(bind_addr, 0, sizeof(struct sockaddr_in));
    bind_addr->sin_family       = AF_INET;
    bind_addr->sin_addr.s_addr  = inet_addr(localAddr);   //inet_ntoa(rtp_addr->sin_addr) get ip string
    bind_addr->sin_port         = htons(localPort);       //rtp_addr(sock->sin_port)
    
    status = bind(rtp_sock, (struct sockaddr *)bind_addr, sizeof(struct sockaddr_in));
    if(status < 0) {
        close(rtp_sock);
...
pj_status_t transport_channel_destroy( struct trans_channel*chan)
{
    ...
    log_debug("transport_channel_destroy done sock:%d", sockfd);
...
pj_status_t transport_thread_start( struct trans_channel*chan, char*threadName)
{
    ...
    pj_thread_create(threadName, (thread_proc *)chan->work_recv, chan->user_udp, 0, 0, &chan->recv_tid);
    if (status != 0) {
...
pj_status_t transport_channel_start( struct trans_channel*chan, const char*remoteAddr, unsigned short remoteRtpPort, char*threadName)
{
    pj_status_t status = 0;

    //transport_remote_connect(chan, remoteAddr, remoteRtpPort);
    transport_thread_start(chan, threadName);
...
pj_status_t transport_udp_start( struct transport_udp* udp) {
    ...
    transport_thread_start(&udp->rtp_chanel,  "rtp_thread");
    transport_thread_start(&udp->rtcp_chanel, "rtcp_thread");
...
{
    pj_status_t status = 0;
    transport_remote_connect(&udp->rtp_chanel, remoteAddr, remoteRtpPort);
    transport_remote_connect(&udp->rtcp_chanel, remoteAddr, remoteRtpPort+1);
    return status;
}
...
ssize_t transport_channel_send(struct trans_channel *rt_channel, void *rtpPacket, pj_uint32_t size)
{
    ...
    if(rt_channel->sockfd<=0)
    {
        log_error("channel send failed, socket is <= 0");
        return -1;
    }
...
//        return size;
    ...
    status = sendto(rt_channel->sockfd, rtpPacket, size, 0, (const struct sockaddr *)&rt_channel->remote_addr, addr_len);
    //pthread_mutex_unlock(&udp->udp_socket_mutex);
    if(status > 0) {
        if(rt_channel->is_rtcp)
            log_debug("rtcp sendto sockid:%d packet size:%d seq:%d",rt_channel->sockfd, status, pj_ntohs(head->seq));
        else
            log_debug("rtp_ sendto sockid:%d packet size:%d seq:%d",rt_channel->sockfd, status, pj_ntohs(head->seq));
    }
    else
    {
							  ...
							  const void *rtpPacket, pj_uint32_t size)
{
    ...
    if(rt_channel->resend && rt_channel->sockfd>0)
    {
        pthread_mutex_lock(&udp->rtp_cache_mutex);
