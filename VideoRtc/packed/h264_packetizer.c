...
PJ_DEF(pj_status_t) pjmedia_h264_packetize_(pjmedia_h264_packetizer *pktz,
                                           pj_uint8_t *buf,
                                           pj_size_t buf_len,
                                           unsigned *buf_pos,
...
{
	int result = -1;
	switch(pktz->cfg.mode)
	{
		case PJMEDIA_H264_PACKETIZER_MODE_SINGLE_NAL:

		break;

		case PJMEDIA_H264_PACKETIZER_MODE_NON_INTERLEAVED:
		break;
		case PJMEDIA_H264_PACKETIZER_MODE_INTERLEAVED:
		break;
	}
	return result;
}
...
static pj_bool_t find_nalu_confirm_fu_packet(pj_uint8_t *buf_tart, pj_uint8_t *buf_end, pkt_rtp_payload_msg *msg)
{
...
static pj_bool_t packetize_rtp_payload_msg(pj_uint8_t *buf, pj_size_t buf_len, pkt_rtp_payload_msg *msg)
{
										   ...
										   unsigned int *payload_len)
{
	...
	if(nalu_len<=0)
	{
		log_debug("_h265 nalu_len is:%d", nalu_len);
		return -1;
	}
	*pos = *pos + msg->start_code * msg->nalu_header_len;
	...
	if(HEVC_NAL_UNIT_VPS == msg->nal_type || HEVC_NAL_UNIT_SPS == msg->nal_type 
	   ...
	   || (!msg->fu_packet))
	{
		log_debug("dym_h265 all nal_len:%d", nalu_len);
 ...
 */
PJ_DEF(pj_status_t) pjmedia_h264_packetize0(pjmedia_h264_packetizer *pktz,
					                       pj_uint8_t *buf,
                                           pj_size_t buf_len,
                                           unsigned *pos,
                                           const pj_uint8_t *payload,
