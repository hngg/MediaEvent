...
typedef struct fu_indicator {
	char F:1;			//F: forbidden_zero_bit 在H.264规范中规定这一位必须为0
	char NRI:2;			//NRI： 2个bit，取00~11似乎指示这个NALU的重要性
	char Type:5;		//5个bit 这个NALU单元的类型
}fu_indicator;

typedef struct fu_header {
	char S:1;			//当设置成1,开始位指示分片NAL单元的开始。当跟随的FU荷载不是分片NAL单元荷载的开始，开始位设为0
	char E:1;			//当设置成1, 结束位指示分片NAL单元的结束，即, 荷载的最后字节也是分片NAL单元的最后一个字节。当跟随的FU荷载不是分片NAL单元的最后分片,结束位设置为0
	char R:1;			//保留位必须设置为0，接收者必须忽略该位。
	char Type:5;		//暂时还没用吧
}fu_header;

typedef struct fu_info {
	fu_indicator 	indicator;
	fu_header 		header;
}fu_info;
...
typedef enum H265_Nal_Unit_Type
{
	...
	HEVC_NAL_UNIT_UNSPECIFIED_49,	//here = 49
...
#define RTP_H265_PAYLOAD_SIZE   970  //PJMEDIA_MAX_VID_PAYLOAD_SIZE	/*should be less than PJMEDIA_MAX_VID_PAYLOAD_SIZE*/
