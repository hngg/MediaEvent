...
MediaCodecDecoder* mediacodec_decoder_alloc(int isDebug, int timeout, YUV_PIXEL_FORMAT yuv_pixel_format, int codecType, void*surface) {
	MediaCodecDecoder* decoder = mediacodec_decoder_alloc1(isDebug, 0, NV12, codecType);
	decoder->surface = surface;
	return decoder;
}

MediaCodecDecoder* mediacodec_decoder_alloc1(int isDebug, int timeout, YUV_PIXEL_FORMAT yuv_pixel_format, int mediacode) {
	if (isDebug) {
	...
	decoder->MIMETYPE_VIDEO_CODE 	= (HEVC_CODEC == mediacode)?"video/hevc":"video/avc";
...
MediaCodecDecoder* mediacodec_decoder_alloc2(int isDebug, int codecType) {
	return mediacodec_decoder_alloc1(isDebug, 0, NV12, codecType);
}

MediaCodecDecoder* mediacodec_decoder_alloc3(int codecType){
	return mediacodec_decoder_alloc2(1, codecType);
}

MediaCodecDecoder* mediacodec_decoder_alloc4(int codecType, void*surface) {
	MediaCodecDecoder* decoder = mediacodec_decoder_alloc3(codecType);
	decoder->surface = surface;
	return decoder;
}
	...
	if(decoder){
		...
		if(decoder-> SDK_INT >= 16)
		{
			if(decoder-> SDK_INT >= 21){
				decoder->MAX_TIME_OUT = 20000;//origin 3000
			}
			else{
				decoder->MAX_TIME_OUT = 30000;//origin 8000
			}
		...
		decoder->codec = AMediaCodec_createDecoderByType(decoder->MIMETYPE_VIDEO_CODE);
		AMediaFormat* mediaFormat = AMediaFormat_new();
		AMediaFormat_setString(mediaFormat, AMEDIAFORMAT_KEY_MIME, decoder->MIMETYPE_VIDEO_CODE);
		...
		status = AMediaCodec_configure(decoder->codec, mediaFormat, (ANativeWindow*)decoder->surface, NULL, 0);
		if(status){
			if (decoder->DEBUG) {
...
int mediacodec_decoder_render(MediaCodecDecoder* decoder, uint8_t* in, int length) {
	...
	if (inputBufferIndex >= 0) {
		...
		while (outputBufferIndex != AMEDIACODEC_INFO_TRY_AGAIN_LATER) {
			...
			else if(outputBufferIndex <= -20000){
				if (decoder->DEBUG) {
					MediaCodec_LOGE("AMEDIA_DRM_ERROR_BASE");
				}
				return size;
			}
			else if(outputBufferIndex <= -10000){
				if (decoder->DEBUG) {
...
int mediacodec_decoder_decode(MediaCodecDecoder* decoder, uint8_t* in, int offset, uint8_t* out, int length, int* error_code) {
	if(decoder){
		MediaCodec_LOGI("mediacodec_decoder_decode");
		if (decoder->DEBUG) {
