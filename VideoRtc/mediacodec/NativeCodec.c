...
typedef media_status_t(*pf_AMediaCodec_setParameters)(AMediaCodec*, const AMediaFormat* format);
...
const char* AMEDIAFORMAT_KEY_VIDEO_BITRATE = "video-bitrate";
...
const char* AMEDIAFORMAT_KEY_MAX_BITRATE = "max-bitrate";
...
struct dlsys
{
	struct {
		...
		pf_AMediaCodec_setParameters _setParameters;
...
{ "AMediaCodec_setParameters", OFF(_setParameters), false },//only for sdk version >= 26
...
media_status_t AMediaCodec_setParameters(
	...
	const AMediaFormat* format)
{
	if(gdlsys.AMediaCodec._setParameters)
		return gdlsys.AMediaCodec._setParameters(codec, format);
